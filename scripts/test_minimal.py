...
if __name__ == '__main__':
    ...
    img_paths = glob.glob('/home/users/haoyi.jiang/repos/GauS3TR/scannetpp_rendered/*/*/*_rendered_view*')
    img_paths.sort()
    ...
    with torch.no_grad():
        for img_path in img_paths:
            ...
            meta_path = '/'.join(img_path.split('/')[:-1]) + '/meta.npz'
            meta = np.load(meta_path, allow_pickle=True)
            img_path.split('/')[-1].split('_')[0]
            if 'DSC' in img_path:
                intrins = meta[img_path.split('/')[-1].split('_')[0]].item()['intrinsics']
            elif 'frame' in img_path:
                intrins = meta['_'.join(img_path.split('/')[-1].split('_')[0:2])].item()['intrinsics']
            intrins = torch.from_numpy(intrins).float().to(device)[None]
            ...
            target_path = img_path.replace('_rendered_view', '_rendered_normal_view')
